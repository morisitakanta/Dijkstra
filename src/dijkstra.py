...
class Dijkstra:
    def __init__(self, grid_map, grid_size, iter_max, motion_size_x=1, motion_size_y=1):
    ...
    def planning(self, start, goal):
        s_start = nd.Node(start)
        s_goal = nd.Node(goal)
        plot = plotting.Plotting(start, goal)
        current_cost = s_start.cost
        node_closed = []
        node_closed.append(s_start)
        if start != goal:
                ...
                for node in node_closed:
                    if node.condition == 0:
                        for motion in self.motion:
                                ...
                                if self.is_goal(nn, s_goal, self.motion_size_x, self.motion_size_y):
                                    path = self.generate_path(nn, s_start, s_goal)
                                    # plot.animation(node_closed, path, "Dijkstra")
                                    return path
        ...
        plot.animation(node_closed, [], "Dijkstra")
        return []
    ...
    def generate_path(self, node_near, start, goal):
        path = [(goal.x, goal.y)]
        node_now = node_near
